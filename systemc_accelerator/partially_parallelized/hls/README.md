# Partially Parallelized Accelerator: HLS and FPGA Flow
...
The configured target is `xczu3eg-sbva484-1-i` with a 10 ns clock. The HLS
...
`xczu3eg-sbva484-1-i`. Important outputs are:
...
`project_bigfpga.tcl` and `vivado_synth_hdc_bigfpga.tcl` retain the larger FPGA
configuration. They are not the default partially parallelized configuration.
