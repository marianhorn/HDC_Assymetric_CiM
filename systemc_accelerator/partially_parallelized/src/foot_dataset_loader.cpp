...
void format_dataset_paths(const char *repo_prefix,
    ...
    std::snprintf(training_emg_path,
                  ...
                  "%s/c_model/data/dataset%02d/training_emg.csv",
                  repo_prefix,
                  dataset_id);
    std::snprintf(training_labels_path,
                  DATASET_PATH_CAPACITY,
                  "%s/c_model/data/dataset%02d/training_labels.csv",
                  repo_prefix,
                  dataset_id);
    std::snprintf(testing_emg_path,
                  DATASET_PATH_CAPACITY,
                  "%s/c_model/data/dataset%02d/testing_emg.csv",
                  repo_prefix,
                  dataset_id);
    std::snprintf(testing_labels_path,
                  DATASET_PATH_CAPACITY,
                  "%s/c_model/data/dataset%02d/testing_labels.csv",
                  repo_prefix,
                  dataset_id);
}
