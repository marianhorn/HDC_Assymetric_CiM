#!/usr/bin/env python3
"""Plot quantizer+GA test accuracy over NUM_LEVELS against GA-only baseline."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "baseline accuracy over NUM_LEVELS."
        )
    )
    parser.add_argument(
        "--phase",
...
def keep_complete_levels(rows: list[dict[str, object]], sources: list[str]) -> list[dict[str, object]]:
    complete_rows = []
    for dataset in sorted({int(row["dataset"]) for row in rows}):
        available_by_source = {
            source: {
        ...
        if any(not levels for levels in available_by_source.values()):
            continue
        complete_levels = set.intersection(*available_by_source.values())
        complete_rows.extend(
            row
            for row in rows
            if int(row["dataset"]) == dataset
            and int(row["num_levels"]) in complete_levels
        )
...
def plot_png(
    ...
    axes[-1, 0].set_xlabel("NUM_LEVELS")
    fig.suptitle(
        f"Quantizer+GA vs GA-only uniform test {args.metric.replace('_', ' ')} "
        f"at dimension {args.vector_dimension}"
    )
    fig.tight_layout()
...
<body>
  ...
  <script>
    ...
    datasets.forEach((dataset, idx) => {{
      ...
      layout[`xaxis${{suffix}}`] = {{ title: idx === datasets.length - 1 ? "NUM_LEVELS" : "" }};
...
def main() -> None:
    ...
    sources = ["baseline", "no_ga_baseline"] + args.binning_modes
    averaged = sample_levels(
        keep_complete_levels(add_dataset_average(aggregate(rows)), sources),
        args.sample_step,
    )
    ...
    output = args.output or DEFAULT_PLOTS_DIR / f"zzboth_vs_ga_baseline_by_levels_{args.phase}_{suffix}.png"
    output = output.resolve()
    output.parent.mkdir(parents=True, exist_ok=True)
    csv_output = (DEFAULT_RESULTS_DIR / f"zzboth_vs_ga_baseline_by_levels_{args.phase}_{suffix}.csv").resolve()
