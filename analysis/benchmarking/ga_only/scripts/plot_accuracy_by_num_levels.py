#!/usr/bin/env python3
"""Plot seed-averaged GA-only accuracy over quantization levels at fixed dimension."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot one GA-only accuracy line per dataset over quantization levels."
    )
    parser.add_argument("--vector-dimension", type=int, default=10000)
    parser.add_argument("--metric", default="overall_accuracy")
    parser.add_argument("--phase", choices=["preopt", "postopt"], default="postopt")
    parser.add_argument("--split", choices=["validation", "test"], default="test")
    parser.add_argument("--min-num-levels", type=int)
    parser.add_argument("--max-num-levels", type=int)
...
def validate_args(args: argparse.Namespace) -> None:
    if (
        args.min_num_levels is not None
        and args.max_num_levels is not None
        and args.min_num_levels > args.max_num_levels
    ):
        raise ValueError("--min-num-levels must be <= --max-num-levels")
...
def load_selected_rows(args: argparse.Namespace) -> list[dict[str, object]]:
        ...
        with path.open(newline="", encoding="utf-8") as result_file:
            ...
            for row in reader:
                ...
                if info_match is None:
                    continue
                if (
                    int(row["vector_dimension"]) != args.vector_dimension
                    or info_match.group("phase") != args.phase
                    or info_match.group("split") != args.split
                ):
                    continue
                num_levels = int(row["num_levels"])
                if args.min_num_levels is not None and num_levels < args.min_num_levels:
                    continue
                if args.max_num_levels is not None and num_levels > args.max_num_levels:
                    continue
                rows.append(
                    {
                        "seed": seed,
    ...
    if not rows:
        raise RuntimeError(
            f"No rows for vector_dimension={args.vector_dimension}, "
            f"phase={args.phase}, split={args.split}"
        )
    return rows
...
def aggregate(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in rows:
        key = (int(row["dataset"]), int(row["num_levels"]))
        groups[key].append((int(row["seed"]), float(row["value"])))

    averaged = []
    for (dataset, num_levels), values in sorted(groups.items()):
...
def print_configurations(
    ...
    num_levels = sorted({int(row["num_levels"]) for row in averaged})
    seeds = sorted({seed for row in averaged for seed in row["seeds"]})
    datasets = sorted({int(row["dataset"]) for row in averaged})

    print("Selected GA-only configurations")
    print(f"  vector_dimension: {args.vector_dimension}")
    print(f"  phase/split: {args.phase}-{args.split}")
    print(f"  metric: {args.metric}")
    if args.min_num_levels is not None or args.max_num_levels is not None:
        print(f"  num_levels range: {args.min_num_levels}..{args.max_num_levels}")
    ...
    print(f"  num_levels ({len(num_levels)}): {', '.join(map(str, num_levels))}")
    print()
    print("All configurations used and their seed coverage:")
    print("dataset num_levels seed_count seeds")
    for row in averaged:
        seed_text = ",".join(map(str, row["seeds"]))
        print(
            f"{int(row['dataset']):7d} "
            f"{int(row['num_levels']):10d} "
...
def write_html(path: Path, averaged: list[dict[str, object]], args: argparse.Namespace) -> None:
    ...
    title = (
        f"GA-only {args.metric.replace('_', ' ')} at "
...
<body>
  ...
  <script>
    ...
    const minLevel = Math.min(...numLevels);
    const maxLevel = Math.max(...numLevels);
    const yMin = {y_min!r};
    const yMax = {y_max!r};
    const xScale = x => margin.left + ((x - minLevel) / Math.max(1, maxLevel - minLevel)) * plotWidth;
    ...
    const xTicks = numLevels.filter((_, idx) => idx % Math.ceil(numLevels.length / 12) === 0);
    if (!xTicks.includes(maxLevel)) xTicks.push(maxLevel);
    ...
    makeSvg("text", {{ x: margin.left + plotWidth / 2, y: height - 22, "text-anchor": "middle", class: "label" }}, "Quantization levels");
    ...
    datasets.forEach((dataset, datasetIndex) => {{
      ...
      const points = numLevels
        .filter(numLevel => meansByDataset[dataset][numLevel] !== undefined)
        .map(numLevel => [xScale(numLevel), yScale(meansByDataset[dataset][numLevel]), numLevel, meansByDataset[dataset][numLevel]]);
      ...
      for (const [px, py, numLevel, value] of points) {{
        ...
        circle.appendChild(document.createElementNS("http://www.w3.org/2000/svg", "title")).textContent =
          `Dataset ${{dataset}}, levels ${{numLevel}}: ${{(value * 100).toFixed(2)}}%`;
      }}
...
def output_range_suffix(args: argparse.Namespace) -> str:
    parts = []
    if args.min_num_levels is not None or args.max_num_levels is not None:
        lo = "min" if args.min_num_levels is None else str(args.min_num_levels)
        hi = "max" if args.max_num_levels is None else str(args.max_num_levels)
...
def main() -> None:
    ...
    output = args.output or (
        DEFAULT_PLOTS_DIR
        / (
            f"ga_only_dimension_{args.vector_dimension}_{args.phase}_{args.split}"
