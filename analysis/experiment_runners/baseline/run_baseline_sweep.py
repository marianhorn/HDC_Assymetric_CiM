...
def build_num_levels_step10_to_500():
    return list(range(10, 501, 10))
...
def append_manifest(manifest_path, run_index, total_runs, item_mem_seed, num_levels, vector_dimension, log_file, duration_sec):
...
def append_combined(combined_output_path, log_path, run_index, total_runs, item_mem_seed, num_levels, vector_dimension):
    with open(combined_output_path, "a", encoding="utf-8") as out_f:
        out_f.write(
            ...
            "USE_GENETIC_ITEM_MEMORY=0, BINNING_MODE=0, OUTPUT_MODE=2"
            " =====\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--level40-dim1024",
        action="store_true",
        help="Run only NUM_LEVELS=40 at VECTOR_DIMENSION=1024.",
    )
    ...
    if sum(1 for selected in selected_modes if selected) > 1:
        raise ValueError(
            ...
            "--levels-step10-10000, or --level40-dim1024"
        )
