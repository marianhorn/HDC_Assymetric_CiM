...
ifdef MODEL_VARIANT
	CFLAGS += -DMODEL_VARIANT=$(MODEL_VARIANT)
else ifdef ENCODER_ROLLING
	# Backward compatibility: map legacy ENCODER_ROLLING to MODEL_VARIANT.
	CFLAGS += -DMODEL_VARIANT=$(ENCODER_ROLLING)
endif
ifdef DOWNSAMPLE
...
ifdef GA_REFINED_BINNING
	CFLAGS += -DGA_REFINED_BINNING=$(GA_REFINED_BINNING)
endif
ifdef BIPOLAR_MODE
...
ifdef GA_PIPELINE
	CFLAGS += -DGA_PIPELINE=$(GA_PIPELINE)
endif
ifdef BINNING_MODE
...
ifdef GA_BINNING_EPSILON
	CFLAGS += -DGA_BINNING_EPSILON=$(GA_BINNING_EPSILON)
endif
ifdef GA_BINNING_ALPHA
	CFLAGS += -DGA_BINNING_ALPHA=$(GA_BINNING_ALPHA)
endif
