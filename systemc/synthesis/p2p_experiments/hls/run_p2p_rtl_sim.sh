...
TB_DEFINES=()
for define in P2P_PAYLOAD_SAMPLE P2P_PAYLOAD_ENCODED P2P_PAYLOAD_FULL P2P_ENCODER_MIMIC P2P_ENCODER_MIMIC_NB P2P_ENCODER_SCALAR_MIMIC P2P_INTERNAL_SOURCE P2P_ACCEL_CMD_MIMIC P2P_VALID_READY_ADAPTER P2P_EXTERNAL_P2P P2P_EXTERNAL_P2P_PASSTHROUGH; do
    if grep -Eq "^[[:space:]]*set_attr[[:space:]]+D[[:space:]]+${define}([[:space:]]|$)" project.tcl; then
        if [[ "$define" == "P2P_EXTERNAL_P2P" || "$define" == "P2P_EXTERNAL_P2P_PASSTHROUGH" ]]; then
            EXTERNAL_P2P=1
        fi
        TB_DEFINES+=("-d" "${define}")
